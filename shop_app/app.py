...
@app.route('/')
def bootloader():
    """Serves the initial HTML page containing only the decryption logic."""
    # This JavaScript is the "key" to unlock your app. It's minified to be less readable.
    # It fetches the encrypted data from /data, decrypts it, and injects it into the page.
    bootloader_script = f"""
        <script>
            const K="{XOR_KEY}";
            ...
            async function I(){{const r=await fetch('/data');const p=await r.json();const h=document.documentElement;h.innerHTML=D(p.h);document.body.innerHTML=D(p.b);L();}}
            function L(){{window.pywebview.api.get_products().then(p=>{{document.getElementById('product-list').innerHTML=p.map(i=>`<div class="bg-white p-4 rounded-lg shadow-md flex items-center space-x-4"><div class="w-16 h-16 pup-bg-burgundy rounded-lg flex items-center justify-center text-white"><i class="fas fa-tshirt"></i></div><div class="flex-1"><h4 class="font-semibold">${{i.name}}</h4><p class="text-sm text-gray-500">₱${{i.price.toFixed(2)}}</p></div><button onclick="handleAddToCart(${{i.id}})" class="bg-red-500 text-white px-4 py-2 rounded-full font-bold text-sm">ADD</button></div>`).join('');}});document.getElementById('login-form').addEventListener('submit',handleLogin);document.getElementById('register-form').addEventListener('submit',handleRegister);}}
    ...
    # We create a new "head" and "body" payload for the bootstrapper to assemble
    head_html = '<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>PUP E-Commerce Shop</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css"><style>:root{--pup-burgundy:#722F37;}.pup-bg-burgundy{background-color:var(--pup-burgundy);}.pup-text-burgundy{color:var(--pup-burgundy);}.page-view,.app-view{display:none;}.page-view.active,.app-view.active{display:block;}.bottom-nav{position:fixed;bottom:0;left:0;right:0;z-index:50;}.content-container{padding-bottom:80px;}.cart-badge{position:absolute;top:-5px;right:-5px;background:#EF4444;color:white;border-radius:50%;width:18px;height:18px;font-size:11px;display:flex;align-items:center;justify-content:center;}</style>'
    body_html = f'<div id="root"></div>{bootloader_script}'
    
    # The final HTML served is just the shell
    return f"<!DOCTYPE html><html lang='en'><head>{head_html}</head><body class='bg-gray-100'>{body_html}</body></html>"
